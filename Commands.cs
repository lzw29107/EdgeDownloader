        ...
        foreach (DownloadInfo downloadInfo in downloadInfoSet)
        {
            ...
            if (downloadInfo.GetFileType() == parsedArguments.FileType)
            {
                ...
                else
                {
                    ...
                    byte[]? sha256Hash = downloadInfo.Sha256 != string.Empty ? Convert.FromHexString(downloadInfo.Sha256) : null;
                    await RequestHelper.DownloadFileAsync(downloadInfo.Url, outputPath, sha256Hash).ConfigureAwait(false);
