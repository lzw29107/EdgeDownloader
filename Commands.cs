...
internal static partial class Commands
{
	internal static void Usage(int exitCode = 0)
	{
		...
		Console.WriteLine("  /version:<ver>    Edge version (e.g., 140.0.3420.0; latest if omitted)");
	...
	internal static async Task DownloadAsync(ParsedArguments parsedArguments)
	{
		...
		{
			Console.WriteLine("\rFailed to generate download links.");
			return;
		}

		Console.WriteLine($"\rTotal download links found: {downloadInfoSet.Count}");
		Console.WriteLine();
		...
		foreach (DownloadInfo downloadInfo in downloadInfoSet)
		{
			...
			if (downloadInfo.GetFileType() == parsedArguments.FileType)
			{
				bool recommended = downloadInfoSet.Count == 1 ||
					...
					(parsedArguments.Arch == SupportedArch.Arm && downloadInfo.FileName.Contains("Tencent"))); // Just temporary workaround for Android Edge Stable links
				...
				else
				{
					...
					await RequestHelper.DownloadFileAsync(downloadInfo.Url, outputPath).ConfigureAwait(false);
	...
	internal static async Task ListAsync(bool all)
	{
		...
		Console.WriteLine($"\rTotal products found: {products.Count}           ");
